#define S2     RA0_bit  //DEFINE ENTRADAS IO
#define S1     RA1_bit
#define led2   RA2_bit
#define led1   RA3_bit
#define servo1 RB0_bit //RB0 IO CONTROLA O SERVO1

//FUNCOES PARA CONTROLAR O SERVO
void servo1_horario();
void servo1_centro();
...
void main(){
    ...
    while(1){
             //Se manter S1 pressionadO o servo move-se no sentido horário
             if(S1 == 0x00 && S2 == 0x01) servo1_horario();
             //Se manter S2 pressionado o servo move-se no sentido anti horário
             else if(S2 == 0x00 && S1 == 0x01) servo1_antihorario();
             //Senão... centraliza servo
             else servo1_centro();
    }
}
void servo1_horario(){
